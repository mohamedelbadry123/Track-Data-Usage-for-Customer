        ...
        public async Task<IEnumerable<CustomerUsageOverviewDTO>> GetCustomerUsageOverviewAsync()
        {
            ...
            return await usageRecords
                ...
                .Select(group => new CustomerUsageOverviewDTO
                {
                    ...
                    PlanLimit = group.First().Customer.CustomerPlan.Plan.DataLimit, 
                    PlanName = group.First().Customer.CustomerPlan.Plan.PlanName, // Concatenate first and last name from the first record in the group
